    ...
    private void loadConfig() throws IOException, InvalidConfigurationException {
        ...
        if (!configFile.exists()) {
            try {
                ...
                Bukkit.getConsoleSender().sendMessage(ChatColor.GRAY + " [ Retro Almas ] " + ChatColor.GREEN + "Configurac\u0327o recarregado: " + configFile.getName());
            } catch (IllegalArgumentException e) {
                // File not found in resources, creating a blank one
                configFile.createNewFile();
                Bukkit.getConsoleSender().sendMessage(ChatColor.GRAY + " [ Retro Almas ] " + ChatColor.GREEN + "Configurac\u0327o criada: " + configFile.getName());
            }
        }
